...
package net.sf.j_ircd.server.auth;
...
public class PropertiesLoginModule implements LoginModule {
	...
	private void authenticate(final String name, char[] passChars, final String host, final String addr, final int port) throws LoginException {
		...
		String[] userInfo = net.sf.j_ircd.server.irc.Util.split(userLine, ' ');
		userLine = null;
		...
		boolean hostSucceeded = net.sf.j_ircd.server.irc.Util.match(userInfo[1], host);
		boolean ipSucceeded = net.sf.j_ircd.server.irc.Util.match(userInfo[2], userInfo[3], addr);
