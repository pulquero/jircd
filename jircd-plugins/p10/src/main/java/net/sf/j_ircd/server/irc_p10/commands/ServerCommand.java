...
import net.sf.j_ircd.server.irc_p10.*;

/**
 * @author markhale
 */
public class ServerCommand extends net.sf.j_ircd.server.irc.commands.ServerCommand {
    ...
    private void login_P10(final UnregisteredEntity src, String[] params) {
        ...
        int hopcount = Integer.parseInt(params[1]);
        if(hopcount != 1)
            net.sf.j_ircd.server.irc.Util.sendError(src, "The hop count must be 1 for a peer server: "+hopcount);
        String startTime = params[2];
        String linkTime = params[3];
        String protocol = params[4];
        String tokenB64 = params[5].substring(0, 2);
